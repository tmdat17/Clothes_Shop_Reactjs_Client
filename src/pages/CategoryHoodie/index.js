...
function CategoryHoodie() {
    ...
    useEffect(() => {
        ...
        getData()
            .then((res) => {
                res.data.map((item) => {
                    if (item.category.type_product === 'ao hoodie') {
                        setAllHoodies((prev) => [...prev, item]);
                    }
                });
            })
